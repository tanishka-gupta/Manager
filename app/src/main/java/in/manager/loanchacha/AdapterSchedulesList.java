...
class AdapterSchedulesList extends BaseAdapter {

		private Activity activity;
		public ImageLoader imageLoader;


		public AdapterSchedulesList(Activity act) {
			this.activity = act;
		...
		public int getCount() {
			...
			return ActivitySchedulesList.Menu_ID.size();
		}
		...
		//	holder.txtSubText = (TextView) convertView.findViewById(R.id.txtSubText);
			...
			holder.txtText.setText(ActivitySchedulesList.Menu_name.get(position));

            String str="";
			//if(ActivitySchedulesList.Menu_price.get(position)==0L) {
//              holder.txtSubText.setVisibility(View.GONE);
          ...
          /*  } else {
				if (ActivitySchedulesList.Menu_price.get(position) < ActivitySchedulesList.Menu_max_price.get(position)) {
					holder.txtSubText.setVisibility(View.VISIBLE);
					str = "<font color='#A80000' >" + ActivitySchedulesList.Currency + ActivitySchedulesList.Menu_max_price.get(position);
					holder.txtSubTextmax.setText(Html.fromHtml(str));
				...
				float max_price = Float.parseFloat(ActivitySchedulesList.Menu_max_price.get(position).toString());
				float price = Float.parseFloat(ActivitySchedulesList.Menu_price.get(position).toString());
				...
				str = ActivitySchedulesList.Currency + " " + ActivitySchedulesList.Menu_price.get(position);
				holder.txtSubText.setText(Html.fromHtml(str));
			} */


            imageLoader.DisplayImage(Constant.AdminPageURL+ActivitySchedulesList.Menu_image.get(position), holder.imgThumb);
